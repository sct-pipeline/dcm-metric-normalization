...
def plot_scatter_grid(df, output_dir):
    ...
    plt.tight_layout()
    fig_path = os.path.join(output_dir, 'scatter_cord_vs_canal_area_perlevel.png')
...
def plot_scatter_grid_by_sex(df, output_dir):
    ...
    for i, level in enumerate(VERTEBRAL_LEVELS):
        ...
        ax.legend()
    plt.tight_layout()
    fig_path = os.path.join(output_dir, 'scatter_cord_vs_canal_area_perlevel_by_sex.png')
    plt.savefig(fig_path, dpi=300, bbox_inches='tight')
    print(f"Figure saved: {fig_path}")
