...
def plot_scatter_grid_by_sex(df, output_dir):
    ...
    for i, level in enumerate(VERTEBRAL_LEVELS):
        ...
        for sex, color in sex_colors.items():
            ...
            sns.scatterplot(x=x, y=y, ax=ax, color=color, alpha=0.6, label=('Male' if sex == 'M' else 'Female'), s=80)
            if len(x) > 1:
                z = np.polyfit(x, y, 1)
                pfit = np.poly1d(z)
                x_vals = np.linspace(x.min(), x.max(), 100)
            ...
            ax.text(0.35 if sex == 'M' else 0.98, 0.02, stats_text, transform=ax.transAxes,
                    verticalalignment='bottom', horizontalalignment='right',
                    bbox=dict(boxstyle='round', facecolor='lightgray', alpha=0.8), fontsize=TICKS_FONT_SIZE-4)
